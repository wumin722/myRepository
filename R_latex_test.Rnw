...
\usepackage[pdftex,dvipdfm,colorlinks=true]{hyperref}%超链接
...
@
\Sexpr{print("$SO_2$")}
本文使用的环境指标数据来自环境保护部发布的\href{http://www.mep.gov.cn/gkml/hbb/bgg/201208/t20120823_235126.htm}{\color{blue}《2012上半年环境保护重点城市环境空气质量状况》}。
<<>>=
library(cluster)
library(xtable)
df <- graph[ , c(2,4,6)]
